...
const Navbar = ({ theme, isDarkMode, setIsDarkMode, setIsInteractive }: NavbarProps) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 flex items-center justify-between px-10 md:px-16 h-20 w-full z-50 border-b backdrop-blur-md transition-colors duration-500 ${theme.nav}`}>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className={`text-2xl font-black tracking-tighter transition-colors duration-500 ${theme.text} leading-none`}>
      ...
      <div className="hidden md:flex items-center gap-10">
        {['About', 'Experience', 'Skills', 'Work', 'Contact'].map((item) => (
          <motion.a 
            ...
            className="text-sm font-black uppercase tracking-tight cursor-none"
          >
            {item}
          </motion.a>
        ))}
      ...
      <div className="flex items-center gap-6">
        <button 
          ...
          onMouseEnter={() => setIsInteractive(true)}
          onMouseLeave={() => setIsInteractive(false)}
          className={`p-2.5 rounded-xl transition-all duration-500 border shadow-sm active:scale-90 ${
            isDarkMode 
